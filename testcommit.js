// curl --header  'Authorization: token ' + token --location https://api.github.com/repos/PageDragonDev/abouttally/git/refs/heads/master
const fetch = require('node-fetch');

const baseRepoURL = 'https://api.github.com/repos/PageDragonDev/abouttally/git/';

let createFiles;
const doWork = async (token, branch, files) => {
    // GET REFERENCE
    ...
    if (refResponse.status !== 200) {
        console.error('Error getting reference:', refResponse.status);
        return;
    }
    ...
    const newTreeResult = await createFiles(token, baseTreeSHA, files);
...
createFiles = async (token, baseTreeSHA, files) => new Promise(async (resolve, reject) => {
    // CREATE BLOBS
...
doWork('3ae8ccb9e4f4744caba1b0aebcae0d18ab4d4e34', 'master', fileInfo);
