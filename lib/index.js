...
exports.update = (token, repo, branch, files, message = 'GitHub Direct Commit') => new Promise(async (resolve, reject) => {
    ...
    const newTreeResult = await createFiles(repo, branch, token, baseTreeSHA, files);
...
exports.exists = async (token, repo, branch, path) => new Promise(async (resolve) => {
    const baseTestURL = `https://api.github.com/repos/${repo}/contents`;

    // TEST EXISTENCE

    const testFile = `${baseTestURL}/${path}?ref=${branch}`;
...
createFiles = async (repo, branch, token, baseTreeSHA, files) => new Promise(async (resolve, reject) => {
    const baseRepoURL = `https://api.github.com/repos/${repo}/git/`;
    const baseTestURL = `https://api.github.com/repos/${repo}/contents`;
    ...
    for (let fileIdx = 0; fileIdx < files.length; fileIdx++) {
        ...
        const testFile = `${baseTestURL}/${file.path}?ref=${branch}`;
        const res = await fetch(testFile, { // eslint-disable-line
            method: 'GET',
            headers: {
                Authorization: `token ${token}`,
...
exports.merge = (token, repo, base, head, message = 'GitHub Direct Merge') => new Promise(async (resolve, reject) => {
    const baseRepoURL = `https://api.github.com/repos/${repo}/merges`;
    ...
    const newMergeBody = JSON.stringify(merge);
    const newMergeResponse = await fetch(`${baseRepoURL}`, {
        method: 'POST',
        headers: {
            Authorization: `token ${token}`,
        },
    ...
    if (newMergeResponse.status !== 200 && newMergeResponse.status !== 201) {
        console.error('Error posting merge:', newMergeResponse.status);
        reject(new Error('Error posting merge:', newMergeResponse.status));
        return;
    }
    const newMergeResult = await newMergeResponse.json();
    resolve(newMergeResult);
});
