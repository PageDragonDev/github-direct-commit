...
exports.merge = (token, repo, base, head, message = 'GitHub Direct Merge') => new Promise(async (resolve, reject) => {
    ...
    if (newMergeResponse.status !== 200 && newMergeResponse.status !== 201 && newMergeResponse.status !== 204) {
        console.error('Error merging ' + base + ' to ' + head + ':', newMergeResponse.status);
        reject(new Error('Error merging ' + base + ' to ' + head + ':', newMergeResponse.status));
        return;
    }
